 ...
 void drive(int inches, float waitTimeSec, float speed)
  {
    ...
    float kP_dist = .9;
    float kI_dist = 1.4; //was.1
    float kD_dist = .6; //was.5
    ...
    con.Screen.print("");
///*abs(derivative_dist)>20*/  // || rMotor1.position(deg)<ticks) && derivative_dist>integral_dist)
    while((T1<timeLimitMsec)) // || rMotor1.position(deg)<ticks) && derivative_dist>integral_dist)
    {
